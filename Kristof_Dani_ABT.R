...
# set.seed(10)
# users_sample <- data %>% select(by) %>% unique() %>% sample_n(20000)
# sample <- data %>% filter(by %in% users_sample$by)
data <- readRDS(paste0(path, "collected/items_185_245.RDS"))
sample <- data
...
ABT <- ABT %>% transform(is_active = activity_number > 0) %>%
  ...
  arrange(by, year_month) %>%
  mutate(active_prev_month = lag(is_active), prev_by = lag(by)) %>%
  filter(active_prev_month == TRUE, by == prev_by) %>%
  mutate(churn = !is_active) %>%
  select(-is_active, -active_prev_month, -prev_by)
